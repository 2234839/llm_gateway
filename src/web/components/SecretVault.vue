...
/** 新建表单（占位符由后端自动生成，不可自定义） */
const form = ref<{ name: string; value: string; enabled: boolean }>({ name: "", value: "", enabled: true })
/** 真实值显示/隐藏 */
...
function startCreate() {
  ...
  creating.value = true
  form.value = { name: "", value: "", enabled: true }
}

function startEdit(s: SecretInfo) {
  editingId.value = s.id
  creating.value = true
  form.value = { name: s.name, value: s.value, enabled: s.enabled }
}
...
<template>
  <div class="secret-vault">
    ...
    <details v-if="!loading && !creating" class="vault-principle">
      <summary>{{ t('vault.principleTitle') }}</summary>
      <ol class="principle-list">
        <li>{{ t('vault.principleStep1') }}</li>
        <li>{{ t('vault.principleStep2') }}</li>
        <li>{{ t('vault.principleStep3') }}</li>
        <li>{{ t('vault.principleStep4') }}</li>
      </ol>
      <p class="principle-note">{{ t('vault.principleNote') }}</p>
    </details>
      ...
      <div v-if="creating" class="form-card">
        ...
        <div class="form-grid">
          ...
          <label>
            ...
            <input :value="t('vault.placeholderAutoGenerated')" disabled />
          </label>
